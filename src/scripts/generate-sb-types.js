...
const generateStoryblokTypes = async () => {
  ...
  execSync(
    `cd ${CUSTOM_BUILD_CACHE_DIR} && npx storyblok pull-components --space=${process.env.SB_SPACE_ID}`
  );

  storyblokToTypescript({
    componentsJson: require(path.join(
      process.cwd(),
      `${CUSTOM_BUILD_CACHE_DIR}/components.${process.env.SB_SPACE_ID}.json`
    )),
    path: path.join(process.cwd(), "src/types/sb-components-schema.ts"),
